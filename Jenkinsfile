        ...
        stage('DeployToProduction') {
            steps {
                ...
                withCredentials([sshUserPrivateKey(credentialsId: 'backend_ssh', usernameVariable: 'USERNAME', keyFileVariable: 'SSH_KEY')]) {
                    script {
                        sh " ssh -To -i $SSH_KEY StrictHostKeyChecking=no $USERNAME@backend_ip \"docker pull n0nce/tennis-server:${env.BUILD_NUMBER}\""
                        try {
                        ...
                        sh " ssh -To -i $SSH_KEY StrictHostKeyChecking=no $USERNAME@backend_ip \"docker run --restart always --name tennis-server -p 5000:5000 -d n0nce/tennis-server:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }
